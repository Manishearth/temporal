...
impl Tzif {
    ...
    pub fn get(&self, epoch_seconds: &Seconds) -> TemporalResult<LocalTimeRecord> {
        let search_result: Option<LocalTimeRecord> = self
            ...
            .map(|search_result| {
                ...
                match search_result {
                    TransitionTimeSearch::Index(idx) => Some(get_local_record(db, idx - 1).into()),
                    TransitionTimeSearch::PosixTz => None,
                }
            })
            .ok_or(TemporalError::general("No transition time found."))?;

        let Some(record) = search_result else {
            // Need to resolve the POSIX tz string.
            let Some(posix_tz_string) = self.0.footer.as_ref() else {
                return Err(TemporalError::assert()
                    .with_message("POSIX tz string is needed to resolve time zones."));
            };

            return resolve_posix_tz_string_for_epoch_seconds(posix_tz_string, epoch_seconds.0);
        };
        Ok(record)
    }

    // There are various other ways to search rather than binary_search. See glibc
    pub fn binary_search(&self, epoch_seconds: &Seconds) -> Option<TransitionTimeSearch> {
        self.0.data_block2.as_ref().map(|b| {
            match b.transition_times.binary_search(epoch_seconds) {
                Ok(idx) => TransitionTimeSearch::Index(idx),
                Err(idx) if idx == 1 => TransitionTimeSearch::Index(idx),
                // Err can be here means that the POSIX tz string needs to be resolved.
                Err(_idx) => TransitionTimeSearch::PosixTz,
            }
        })
    }
    ...
    pub fn v2_estimate_tz_pair(&self, seconds: &Seconds) -> TemporalResult<LocalTimeRecordResult> {
        ...
        let TransitionTimeSearch::Index(estimated_idx) = self
            .binary_search(seconds)
            .expect("Data block to be valid.")
        else {
            // Handle Posix Tz String
            let Some(posix_tz_string) = self.0.footer.as_ref() else {
                return Err(TemporalError::assert());
            };

            return resolve_posix_tz_string(posix_tz_string, seconds.0);
        };
        ...
        let Some(ref data_block) = self.0.data_block2 else {
            return Err(TemporalError::general(
                "Data block must exist to search for time zones.",
            ));
        };
        ...
        match offset_range.contains(&current_diff.0) {
            ...
            true => Ok((initial_record, next_record).into()),
